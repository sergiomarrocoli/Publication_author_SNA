...
AuthorsList <- distinct(Publications[1:2])           # select useful columns (Author and Title)
AuthorsList$Author <- str_replace_all(AuthorsList$Author,"[^[:graph:]]", "") 
AuthorsList$Author <- gsub(" ", "", AuthorsList$Author, fixed = TRUE)   # remove white space
AuthorsList$Author <- tolower(AuthorsList$Author)                       # make lowercase
...
for (i in Authors){
  # make new column with name of discipline
  AuthorsList[i] <- (grepl(i, AuthorsList$Author)) * 1 # make new variable ([] = $ for index), grepl makes TRUE/FALSE, multiply by 1 gives ones and zeros
}
